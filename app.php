...
$app->get('/', function () use ($app) {
    ...
    // [END add_tasks]
    // [START url_endpoints]
    //(new PushTask('/worker', ['data_for_task' => 1234]))->add();
    // [END url_endpoints]
...
$app->post('/worker', function (Request $req) use ($app) {
	...
	//file_get_contents("https://api.telegram.org/bot$telegramapi/sendMessage?text=test&chat_id=$telegramchat");
	//echo 'kopet';
    //return 'name: ' . $req->get('name') . "\n"
     ///   . 'action: ' . $req->get('action');
     shell_exec('mkdir kopet');
     unlink('kopet');
});

$app->get('/kopet', function (Request $req) use ($app) {
